...
// to check if is admin who enter and the admin is mohammad with id = 1
if (isset($_SESSION['userloged']) && $_SESSION['userID'] == 1) {
    $pageTitle =  "Admins";
    include 'init.php';
...
<div class="container">
    ...
    <a class="btn btn-danger fixedbtn" href='?do=add'> <i class="fa fa-plus"></i> Add New Member </a>
</div>
...
<?php // End Add Page
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($formErrors)) {
                ...
                if ($checkUsername > 0) {
                    ...
                    // add as admin the GroupID == 1
                    $stmt = $con->prepare("INSERT INTO users (Username,Password,Email,FullName,GroupID,RegStatus,Date) VALUES (:user , :pass , :email , :fname, 1,1,now())");
                    $stmt->execute(array(
                        ':user'     => $username,
                        ':pass'     => $hashPass,
                        ':email'    => $email,
                    ...
                    if ($stmt) {
                        ...
                        redirectHome($theMsg, "adminManage.php", 2);
                    }
                }
            }
        } else {
        ...
        if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] != 1) { // select admin based  on  id
            $user_id = $_GET['id'];
...
<?php
        ...
        $user_id = isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] != 1 ? intval($_GET['id']) : 0;
        $check = checkItem('UserID', 'users',  $user_id);

        // all above to check if the id in link is correct

        if ($check > 0) {
            $stmt =  $con->prepare("DELETE FROM users WHERE UserID = :userid  AND  UserID != 1");
